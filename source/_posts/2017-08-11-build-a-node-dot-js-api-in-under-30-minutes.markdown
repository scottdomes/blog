...
comments: true
categories: [Nodejs, JavaScript, Web Development, Programming, API]
---
...
![](https://cdn-images-1.medium.com/max/800/1*s5LVdcugM62xzSvGUpTLWA.png)
...
fat-arrow functions), but nothing too complex.

For this tutorial, you’ll create the skeleton of a back-end for a note-taking
application — think [Google Keep](http://keep.google.com/). You want to be able
to do all four CRUD actions on your notes: create, read, update, and delete.

### Setting Up

If you don’t have Node installed, [see
here](https://howtonode.org/how-to-install-nodejs).

In a new directory, run npm init, and follow along with the prompts, giving your
app the name of ‘notable’ (or whatever else you might like).
```
    npm init
```
Once that’s done, you should have a *package.json *ready to go in your
directory. This means you can start installing the dependencies you need for
your project.

You’re going to use Express as your framework, MongoDB as the database, and a
...
little package that automatically restarts your server when files change.
...
``` javascript
    ...
    const app            = express();
```
You’re going to use the MongoClient to interact with your database. Note that
...
```
Now if you run *npm run dev *(or *node server.js* if you didn’t install Nodemon)
...
```
These directories may seem like overkill for your simple small app, but it’s
...
```
Note that since you don’t have a database yet set up, you’re just passing in an
empty object.
...
```
When the app receives a *post *request to the ‘/notes’ path, it will execute the
...
```
Last step to your preliminary route: set up the database, and then add your data
in.
...
```
Don’t forget to add your username and password (the ones from the database user,
...
```
Once the insert is complete (or has failed for whatever reason), you want to
...
```
Try it out! Send an x-www-form-urlencoded POST request with Postman, with a
...
```
Just like before, you’re going to call a method on your database collection of
...
```
You can grab the id from the URL parameters via *req.params.id*. However, if you
...
![](https://cdn-images-1.medium.com/max/800/1*LZ1DzuxzAJAyNYjO4sZcmg.png)

Note the imperfection with this code- if you fail to supply a body or title, the
PUT request will nullify those fields on the note in the database.

You could easily add some conditional logic to update the fields only if they’re
present in the request- I left that out just to keep it simple.

### API Complete

That’s it! You have a working Node API with each of the four major CRUD
operations.

The goal of this tutorial was to give you a degree of familiarity with Express,
